...
1. <h3>[Введение](#Введение)</h3>
    ...
    - [Подключение к сервису запущенному на host машине вне docker](#Подключение-к-сервису-запущенному-на-host-машине-вне-docker)
...
- `docker system prune` - удаляет все images, containers, networks (при 
    ...
    остановленные контейнеры вместе с их ресурсами.
...
- `docker run -i -t app /bin/bash` - создает контейнер на основе образа `app` и 
    запускает там `bash`
- `docker exec -i -t af398085b30c /bin/bash` - выполняет подключение к 
    запущенному контейнеру и запускает там `bash`, чтобы можно было выполнять 
    команды внутри конйтера
...
- `docker-compose up -d --no-deps --build app` - пересобирает образ сервиса 
    ...
    (полезна при выкатывании обновлений)
- `docker-compose up --build -d` - пересобирает все сервисы (полезна при 
    выкатывании обновлений)

**Логи**

- `docker logs --tail 100 <id>` - выводит 100 строк логов контейнера `<id>`
...
sudo так, чтобы от root можно было запустить только 1 конкретную программу. 
...
<a name='Подключение-к-сервису-запущенному-на-host-машине-вне-docker'></a>
### Подключение к сервису запущенному на host машине вне docker

При разработке иногда возникает необходимость подключиться к сервису запущенному
на host машине вне docker. Например, к базе данных, которая запущена на host.
Сделать это можно при помощи специального DNS имени `host.docker.internal`, которое
резолвится в ip адрес host машины.

> Эта функциональность предназначена только для целей разработки и не будет работать вне 
> окружения Docker Desktop.

Полезные ссылки:

- [Explore networking features | Docker Documentation](https://docs.docker.com/desktop/networking/#i-want-to-connect-from-a-container-to-a-service-on-the-host)
...
09.04.2020 ничего не изменилось в данный момент поддержка секретов все еще 
...
[Build secrets in Docker Compose, the secure way - pythonspeed](https://pythonspeed.com/articles/build-secrets-docker-compose/).

Ссылки описывающие эксперементальное решение:
