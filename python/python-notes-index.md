...
3. <h3>[Стандартная библиотека](#Стандартная-библиотека)</h3>
...
5. <h3>[Функции](#Функции-раздел)</h3>
...
8. <h3>[Сборщик мусора](#Сборщик-мусора)</h3>

    - [Как работает сборщик мусора (gc)](#Как-работает-сборщик-мусора-(gc))
    
9. <h3>[Многозадачность](#Многозадачность)</h3>
...
11. <h3>[Архитектура](#Архитектура)</h3>

    - [Проблема маленьких классов](#Проблема-маленьких-классов)

12. <h3>[Deploy web приложений](#Deploy-web-приложений)</h3>
...
>значение поля ob_size C-структуры `PyVarObject`, которая представляет в памяти
>любой встроенный объект. Благодя такой реализации, функция len со встроенными
>типами работает очень быстро, так как обращение к полю гораздо быстрее работает 
>чем вызов метода.
...
В отличи от потоков, сопрограммы выполняют только полезную работу, а 
...
<a name='Проблема-маленьких-классов'></a>
### Проблема маленьких классов
