...
> У встроенных в python типов данных (list, str, bytearray и т д) реализация 
> метода `__len__` возвращает значение поля `ob_size` C-структуры `PyVarObject`, 
> которая представляет любой встроенный объект.
