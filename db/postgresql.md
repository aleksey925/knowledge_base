...
- [Индексы](#Индексы)
  ...
  - [Устройство B-Tree индексов](#Устройство-B-Tree-индексов)
...
Составными могут быть индексы только B-tree, GiST, GIN и BRIN. Составные индексы могут использоваться, как когда в 
условии фигурирует только часть полей, так и когда в условии используются все поля входящие в индекс. Главное нужно 
помнить, что для каждого типа есть свои особенности.
...
<a name='Устройство-B-Tree-индексов'></a>
### Устройство B-Tree индексов

Поговорим об особенностях индексов в PostgreSQL и тем, что их отличает от других РСУБД. Речь пойдёт о стандартных для 
реляционных СУБД B-tree индексах:

1. Индексы, строго говоря, являются B+ деревом, что даёт возможность прочитать страницы одного уровня, не возвращаясь 
   каждый раз по иерархии к корневому узлу.

2. Индексы в СУБД PostgreSQL не содержат ссылку на кластерный индекс или первичный ключ, как в некоторых других (MySQL, 
   MS SQL Server). Вместо этого, индекс в PostgreSQL содержит указатель на строку таблицы на диске (TID).

3. Ещё одно отличие индексов в PostgreSQL: индекс не содержит информации об актуальной версии, поэтому нет возможности 
...
- [Особенности индексов в PostgreSQL](https://otus.ru/nest/post/1082/)
